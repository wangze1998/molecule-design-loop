...
- **GEMINI_SCOUT_PRIMARY = true** — Use Gemini via the local `gemini-research` MCP bridge as the primary literature scout and triage engine.
...
For literature search and literature review tasks, Gemini is the default search/review engine.

Use Gemini first to:
- search Zotero-backed literature when available
- build a compact core paper set
- add a small recent-paper correction set
- explain why each paper matters before any broad local/tool sweep

Use local PDFs, arXiv scripts, DeepXiv, and web search as:
- targeted fetch/retrieval tools
- verification tools
- fallback paths if Gemini is unavailable or incomplete
...
### Step 0: Gemini literature scout first

Before doing any broad local or web sweep, ask Gemini to produce a literature evidence packet for the topic.

Prefer this route in Codex:
- `mcp__gemini-research__literature_search_start` for long-running searches
- `mcp__gemini-research__literature_search_status` to poll until completion
- `mcp__gemini-research__literature_search` when you want a direct synchronous result

Default Gemini request:
- Use Zotero as the primary evidence base if available
- Curate `8-12` core papers grouped as:
  - closest prior work
  - strongest baselines
  - same-tier venue exemplars
  - key evidence/mechanism references
- Add `3-5` highly relevant recent papers not already in the Zotero core set
- Return a compact table with title, venue, year, why-relevant, and whether the paper is already in Zotero

Only after Gemini returns this packet should you scan local files or fetch specific papers.
...
### Step 1: Search (external, targeted fallback/verification)
...
- Use this step to verify, expand, or fetch around the Gemini-generated evidence packet rather than starting from scratch
